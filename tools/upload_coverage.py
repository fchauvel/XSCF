...
class CodacyService:
    ...
    def _to_json(self, coverage):
        ...
        if self._store_json:
            with open("payload.json", "w") as log:
                log.write(json.getvalue())
...
class GcovUploader:
    ...
    def upload_coverage(self, source):
        ...
        status, reason, response = self._codacy.send_coverage(gcov_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", dest="output", help=" the JSON payload into 'payload.json'", action="store_true")
    ...
    uploader = GcovUploader(
        CodacyService(user, project, commit, project_token, store_json=store_json))
